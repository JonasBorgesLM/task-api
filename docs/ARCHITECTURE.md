...
- **CSRF rejection (`403`) doesn't use the `{"error": "..."}` envelope every other response in this API guarantees** — `moat/csrf`'s default rejection handler writes plain text `Forbidden` with `Content-Type: text/plain` instead. The fix is `csrf.WithErrorHandler`, wired in `cmd/api`'s `newServer` alongside the rest of the `csrf.Protector` construction, to write the standard envelope. Deferred rather than done alongside the rest of Fase 12 because it's a `moat` library seam, not just an application-level change, and the plain-text response is otherwise harmless — it still carries the right status code and `X-Request-Id`. See `docs/openapi.yaml`'s `Forbidden` response component.

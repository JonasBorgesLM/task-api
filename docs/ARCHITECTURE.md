...
- **`Repository.FindAll` owns ordering, pagination, and (for `task`) the ownership and status/priority filters** — not `Service` — so `postgresRepository` can push `WHERE user_id = … AND status IN (…) ORDER BY … LIMIT … OFFSET …` into the query instead of fetching the whole table (or another user's rows) on every list request. The status/priority filters take *sets*: values within one field match with `OR`, the two fields combine with `AND`, and the filter is applied before the `LIMIT`/`OFFSET` window, so a filtered page windows the filtered set rather than the full table. `Service` validates and de-duplicates the values first, which is what bounds the `IN` list to the enum's own size.
...
- **A caller-supplied ID's *shape* is validated by `Service` (`isValidID`), before either `Repository` implementation is consulted.** Every `task.Service` method taking an existing task's ID checks it against a UUID pattern first; `attachment.Service.Upload`/`Download`/`ListByTask` do the same for `taskID`/`storageKey`. A malformed value maps to the same `ErrNotFound`/`ErrTaskNotFound` a well-formed-but-unknown ID already produces — not a distinct "malformed" error, which would leak that a well-formed one exists. This is what makes `memoryRepository` and `postgresRepository` agree on a malformed ID: without it, PostgreSQL's `::uuid` cast rejected the value as a query error (`500`) while the in-memory store answered `404` for the identical input — see `TestPostgres_FindByID_MalformedID`, which still pins `Repository`'s own unchanged behavior when called directly, bypassing `Service`.
...
- **The cost was a DB read per authenticated request** (`FindSessionByTokenHash`) instead of pure in-process verification. `user.Service.ValidateToken` now checks a short-lived, in-process `tokenCache` first (`internal/user/token_cache.go`, `tokenCacheTTL = 2s`) and only falls through to `Repository` on a miss, so the database read happens at most once per token every two seconds rather than on every request. `Logout`, `LogoutAll`, `ChangePassword` and `DeleteAccount` invalidate the cache immediately in the process handling the call; the only staleness the TTL still allows is a revoked session validating on a *different* process for up to those 2 seconds — see `docs/DECISIONS.md` § "Cache de ValidateToken" for why that bound was chosen and what topology it actually protects against.
...
**Login, login failure, logout-all, password change and account deletion each emit a structured audit log line** (issue #223, `user.Handler.logAuditEvent`) — `event_type`, the account (user ID once authenticated, the normalized email attempted for a login outcome), the caller's address (via the new `middleware.RealIP`, reusing the same address resolution the rate limiters already key on — see "Rate limiting" above), and the request ID, at Info level via the same `*slog.Logger` every other call site uses. No dedicated audit sink: `crierTeeHandler` (`cmd/api/crier.go`) already mirrors every log record to the configured OTLP collector, so this needed no new plumbing, only the five call sites and where the caller's address comes from. Never logs a password, a session token, or a token hash. See `docs/DECISIONS.md` § "Trilha de auditoria".

**`GET`/`DELETE /v1/auth/sessions{/id}` fill the gap between "this session" and "every session"** (issue #224) — a caller can see every active session (newest first, which one is `is_current`) and revoke exactly one, instead of only `POST /auth/logout` (this session) or `POST /auth/logout-all` (all of them). Each session is addressed by an opaque id `deriveSessionID` computes from its `TokenHash` on demand — a second, domain-separated SHA-256 application, never a stored column — rather than the token or its hash itself. See `docs/DECISIONS.md` § "Tela de sessões ativas" for why that was chosen over adding an `id` column, and the cost it accepts (`RevokeSession` scans the caller's own sessions, bounded by `AuthMaxSessionsPerUser`, rather than a direct indexed lookup).
...
**None of the three tiers protects one specific account.** A distributed attacker trying passwords against a known email, one address per attempt, presents each address with few attempts — exactly the profile the auth tier treats as normal traffic. `user.Service.Authenticate`'s `loginBackoff` (issue #220) closes that gap separately: a per-normalized-email failure counter that adds an increasing delay (250ms doubling to a 4s cap) rather than a hard lock, applied to *every* outcome — unknown email, wrong password, or a correct one — so the delay itself never reveals which accounts are currently under contention. Same in-process, single-instance shape as the limiters above. See `docs/DECISIONS.md` § "Atraso progressivo por conta" for the curve, both pitfalls it closes, and what it deliberately doesn't (concurrent requests aren't fully serialized per account).
...
### Listing metadata: `X-Total-Count` and `GET /tasks/stats`

**`GET /tasks` sets `X-Total-Count`** (issues #237/#238) — the number of tasks matching the request's `status`/`priority` filter, independent of `limit`/`offset`. Computed by `Repository.CountAll`, a `COUNT(*)` pushed into the same query shape `FindAll` already uses (`buildTaskFilterWhere`, shared by both so they can never quietly disagree on what "matches the filter" means) — never a second `FindAll` counted in Go. Set before the `ETag`/`If-None-Match` check, so it is present on both a `200` and a `304`: the page's `ETag` only reflects the rows actually returned, so it can stay identical across two requests where the total genuinely changed (a task added on a different page). Chosen as an additive header over a `{data, meta}` envelope specifically to avoid changing `GET /tasks`'s existing response shape — see `docs/DECISIONS.md` § "Total real na listagem" for the measured cost (1-3ms at a realistic heavy scale, via the existing `idx_tasks_user_id_created_at_id` index) and why it's unconditional rather than gated behind an opt-in parameter.

**`GET /tasks/stats`** answers a different question than a page's `X-Total-Count` — counts grouped by status and by priority, across the caller's entire filtered set. `Repository.CountByStatusAndPriority` computes both as `GROUP BY` queries and always populates every `Status`/`Priority` enum key, including at `0`, so a client never has to tell "no tasks in this group" apart from "this key is simply absent." Registered as `GET /tasks/stats`, a literal segment Go's `ServeMux` (1.22+) prefers over the `GET /tasks/{id}` wildcard at the same position regardless of registration order — confirmed directly by `TestTaskStats_Handler_RoutedCorrectly` rather than assumed from the rule.

### CSV export: `GET /tasks/export`

**Streams the caller's entire filtered task set as RFC 4180 CSV** (issues #239-#243), never windowed by `limit`/`offset` — an export exists specifically to return everything a filter matches, unlike a page. `Service.ExportTasks` checks the filtered total via `Repository.CountAll` *before* calling `FindAll`, rejecting with `400` up front (never a response that starts streaming and cuts off mid-file) if the total exceeds a fixed row cap sized to stay well inside the server's `WriteTimeout` — see `docs/DECISIONS.md` § "Exportação CSV de tasks" for the full set of decisions (server-side generation over client-side pagination, a dedicated route over `Accept` negotiation, the row cap's exact number, and the UTF-8 BOM trade-off) and why each was made.
...
### Print report: `web/`'s `/report` route

**A dedicated route (issues #245/#246), not `@media print` on the task list itself** — `GET /v1/tasks` returns at most `PAGE_SIZE` (10) rows a page, so printing the screen directly would silently show only the first page with nothing telling the reader more exist. `ReportPage.tsx` fetches the entire filtered set instead, the same way the CSV export's row cap check does *not* apply here: this route calls `GET /v1/tasks` with no `limit` — the already-documented "absent limit means no limit" behavior `/v1` promises (`cmd/api/main.go`'s `deleteAccountCascade` already relies on the same capability internally), not a new one. See `docs/DECISIONS.md` § "Relatório de impressão" for why this needed no new backend endpoint and the full reasoning against `@media print`.

The filter lives in the URL (`/report?status=&priority=`) rather than being passed in as props from `TaskList` — this address is meant to be printed, bookmarked, or reloaded on its own, and each of those has to reproduce the same report, not whatever `TaskList` last held in memory. The page renders outside `AppShell` entirely (no account menu, theme toggle, or product mark to hide via print rules) and its own `ReportPage.module.css` neutralizes the status/priority badges' color under `@media print` — a printed page can't be trusted to keep color (background-printing settings vary by browser, and hue survives grayscale conversion unevenly), so print rules fall back to a plain bordered box and let the label text alone carry the meaning.

### Link shortening: `internal/link` (opt-in, `LINK_SHORTENING_ENABLED`)

**`task-api`'s first consumer of `github.com/JonasBorgesLM/cairn`** (issues #209-#217) — the whole subsystem is absent, not present-and-rejecting, unless `LINK_SHORTENING_ENABLED` is set (default `false`): no routes registered, no `cairn.Shortener` even constructed. See `docs/DECISIONS.md` § "Encurtador de links" for the five real decisions this integration required and the reasoning behind each.

**cairn occupies the Service-layer domain-logic position, not the Repository one.** `internal/link/service.go`'s `Service` wraps `*cairn.Shortener`, and its entire job is the one thing cairn deliberately does not do: an ownership check on `Revoke` (cairn's own ADR-0010 — the library has no user model, so baking a check in would mean guessing at an authorization model that fits some hosts and is wrong for others). `Service.Revoke` resolves the link first — which already fails with the same uniform shape a code that's invalid, unknown, expired, or revoked gets — and only then compares `OwnerID` against the caller, so a wrong-owner attempt is indistinguishable from "not found" at every layer, matching `.claude/rules/go-domain-errors.md`'s existing rule for every other domain package. Verified with a manual negative control (removing the comparison, watching the dedicated test fail, restoring it) both at the `Service` level and, separately, through the real HTTP surface in `cmd/api`.

**Four routes, three shapes.** `POST/GET /v1/links` and `DELETE /v1/links/{code}` are ordinary authenticated JSON routes, wrapped in `requireAuth` exactly like every other domain package's — `POST /v1/links` needs no dedicated rate-limit tier, since it already goes through the same per-user `moat/ratelimit` tier every other mutating route does, which is the layer cairn's own threat model assumes closes creation abuse (cairn carries no request counter of its own, by design). `GET /{code}` — the public resolve/redirect route — is different on purpose: unauthenticated by design (a short link has to work for a visitor with no session), and deliberately mounted **outside `/v1`**, directly on the same outer `mux` `/health` sits on, so it inherits the global address-keyed rate limiter through `root`'s catch-all without needing its own. It is registered as `mux.Handle("/{code}", ...)` — a single-path-segment wildcard, not a bare `"/"` catch-all — so `ServeMux`'s literal-over-wildcard preference (the same rule `GET /tasks/stats` and `GET /tasks/export` already rely on) keeps it from ever shadowing `/v1/` or `/debug/vars`, regardless of registration order.

**The destination policy closes an SSRF path specific to being a URL shortener.** `policy.Default(ownDomains)` — cairn's own safe-by-default policy — already blocks private/internal networks; `cmd/api/main.go` supplies this deployment's own host (parsed from `LINK_PUBLIC_BASE_URL`) as the one additional domain to block, so the shortener can never be used to launder a link back into its own address space. The TOCTOU gap between that create-time check and DNS at resolve time is a named, accepted residual — see `docs/DECISIONS.md`.

**Rollout is a flag, not a sequence of PRs.** cairn's own integration guide describes four operational rollout steps (memstore behind a flag → a real store in staging → creation for a subset → the public route last, rate-limited from measured traffic) written for a deployment with multiple environments and a real user base to stage against, neither of which this project has. This PR ships the complete feature — all four routes — behind one flag, backed by `memstore` (in-process, no external dependency); swapping in `redisstore` (the backend chosen for the eventual real deployment, see `docs/DECISIONS.md`) is deliberately a separate, later change, since it is the one step that actually needs new infrastructure (Redis, in `docker-compose.yml` and `k8s/`, plus satisfying cairn's own operational contract for it — AOF, a replica, an exercised restore).
...
- **The access log line includes `user_id` for an authenticated request**, absent (not empty) for a public one. This crosses a real ordering obstacle: authentication (`user.RequireAuth`) is wired per-route, inside each domain's `RegisterRoutes`, so it runs *after* `Logging`'s own `next.ServeHTTP` call is already in flight — a plain `context.WithValue` from inside `RequireAuth` would be invisible to `Logging`, which is holding an earlier `*http.Request`. `Logging` instead stashes a pointer to an empty string in the context before calling `next`; `RequireAuth` writes through that pointer via `middleware.RecordUserIDForLog` on a successful authentication. See `Logging`'s doc comment (`internal/middleware/logging.go`) for the full reasoning.
...
- `GET /debug/vars`: stdlib `expvar` — command line, memory/GC stats, goroutine counts, `version`/`commit`, plus `crier_buffer_depth` and `crier_records_dropped` when `CRIER_OTLP_ENDPOINT` is configured (`cmd/api/crier.go`). Baseline visibility with no external dependency; see [Future Improvements](#future-improvements) for a richer metrics surface. Unlike the health routes it requires authentication: those must stay open because an orchestrator's probe carries no credentials, whereas whoever reads runtime internals can present a token.
- **`version`/`commit` under `GET /debug/vars`** identify the exact build a running process came from. Neither can be recovered any other way once the process is running: the image is `FROM scratch` with `.git` excluded from the build context (`.dockerignore`) and the Go binary built with `-trimpath`, so nothing in the container answers "which commit is this" on its own. `Makefile`'s `VERSION`/`COMMIT` (from `git describe`/`git rev-parse` on the host) flow into the image as build `ARG`s and from there into the binary via `-ldflags -X main.version=... -X main.commit=...`; a build with neither set — `go run ./cmd/api`, or `go build` invoked directly — falls back to `"dev"`/`"unknown"`, the same values `main.go`'s package vars default to.
- **Logs mirrored to an OTLP collector, opt-in.** `CRIER_OTLP_ENDPOINT` set makes `run()` wrap the stdout `slog.Handler` in `crierTeeHandler`, which mirrors every record — access log, unexpected-error logs, panic recovery — to the configured collector (e.g. SigNoz) alongside stdout, never instead of it. `core.Crier.Health()` deliberately does not feed `GET /health/ready`: a log backend being unreachable is not a reason to stop sending a healthy replica traffic. `buildCrier` (`cmd/api/crier.go`) also sets `ServiceVersion`, a `Cardinality` guard (bounds the raw `path` attribute's cardinality), and a `Filter` rule that raises the mirrored copy's severity threshold to Error for `/health`/`/health/ready` — routine probe traffic never reaches the collector, a real `503` still does. Full rationale, including why the two sinks coexist, the dependency-cost accounting, and each of these three, in `docs/DECISIONS.md`'s Fase 11 section and the two "crier:" sections after it. `make signoz-dashboard` provisions crier's own SigNoz dashboard template for this service (see `docs/DECISIONS.md`).
...
- **CORS (`middleware.CORS`)**: disabled by default (unset `CORS_ALLOWED_ORIGINS`) when running `go run ./cmd/api` directly — no `Access-Control-*` headers, no behavior change for the server-to-server/same-origin clients this API served exclusively before CORS support existed. `docker-compose.yml` sets it to `http://localhost:8082` by default instead, matching the Swagger UI it bundles at that port, so "Try it out" works with no extra setup — see README's "Swagger UI" section. Positioned inside `Logging` but outside `Recovery` in the middleware chain (`cmd/api/main.go`) so a preflight `OPTIONS` request it answers directly still gets an access log line, and so the `Access-Control-Allow-Origin` header it sets survives even a handler panic. It intercepts `OPTIONS` ahead of `mux` dispatch specifically because a route registered only for `GET`/`POST`/etc. has no `OPTIONS` handler of its own — an unhandled preflight would otherwise `404`. Every allowed-origin response also sets `Access-Control-Expose-Headers: X-Total-Count, ETag` — a response header not on that list is on the wire but invisible to a cross-origin browser client's `fetch` (CORS's own "safelisted response headers" rule), which is exactly how `web/src/features/tasks/useTasks.tsx` reads both. Missing this went unnoticed through the rest of Fase 15 because unit tests mock `fetch` directly, and `web/e2e/pagination.spec.ts` — the one suite that would have caught it, since a total silently read as `0` disables `Next` on a page that should still have one — is real cross-origin (see `playwright.config.ts`'s own doc comment) but neither wired into CI nor run by default locally without the `CORS_ALLOWED_ORIGINS=http://localhost:4173` override that comment documents; a manual cross-origin run against issue #247/15.G1 is what actually surfaced it. See `CHANGELOG.md`'s `[1.6.0]` "Corrigido" entry.
...
### Task list caching: `cistern` (opt-in, `REDIS_ADDR`)

**`GET /v1/tasks` is fronted by a cache-aside decorator, `task.CachedRepository`** (`internal/task/cached_repository.go`), built on `github.com/JonasBorgesLM/cistern` — the whole layer is absent, not present-and-passthrough, unless `REDIS_ADDR` is set: `cmd/api/newServer` wraps `taskRepo` in it only then, the same "absence means the feature does not exist" shape `LINK_SHORTENING_ENABLED` uses for `internal/link`, `DatabaseURL` uses for PostgreSQL. See `docs/DECISIONS.md` § "Cache-aside para GET /v1/tasks" — including its correction paragraph, added after the decision's first draft turned out to overstate what `cistern`'s `Bus` actually guarantees — for the full reasoning.

**The decorator sits below `Service`, not beside it.** `CachedRepository` implements `task.Repository` whole — the same shape `breakerRepository` (`postgres_breaker.go`) already established for wrapping a `Repository` in a cross-cutting concern — so `task.Service`/`task.Handler` stay completely unaware that Redis or `cistern` exist. `FindAll` is the only method actually cached; `FindByID`, `CountAll` and `CountByStatusAndPriority` are pure passthrough to `next` forever. `CountAll` in particular must never be cached: it backs `GET /v1/tasks`'s `X-Total-Count` header, which has to reflect a real, current count on every response (`docs/DECISIONS.md` § "Total real na listagem") — a cached total could disagree with the page actually returned.

**The cache key covers `FindAll`'s full signature, not just the caller.** `docs/DECISIONS.md` § "Índices para status/priority em tasks" already established that no index backs the `status`/`priority` filters — `FindAll` relies on `idx_tasks_user_id_created_at_id` plus in-query filtering — which makes the cache-aside layer the only place that currently reduces that cost. A key built from `userID` alone would silently answer one filter's request with another filter's cached page; `cacheKey` (same file) is a struct of `userID` plus `statuses`/`priorities` — normalized into sorted, comma-joined strings, since a generic `cistern.Cache[K, V]`'s key type must satisfy Go's `comparable`, which slices don't — plus `limit`/`offset`. Invalidation stays coarse on purpose: every cached variant for one user shares a single tag (`user:<id>:lists`), so one `InvalidateTag` call after a write retires all of them at once rather than enumerating filter combinations.

**Invalidation is synchronous in the write call, and fails open.** `Create`/`Update`/`Delete` each invalidate the owner's tag immediately after `next` succeeds — never before, and never at all if `next` itself failed. A publish failure (the `Bus` unreachable, Redis down) is never returned as a write failure: it is reported through `CachedRepository.OnInvalidationError` (wired to `logger.Error` in `cmd/api`, kept out of `Service`/`Handler` entirely) and the worst case degrades to serving a stale L1 entry until its own TTL — mirroring `cistern`'s own reference decorator for this project (`github.com/JonasBorgesLM/cistern/examples/taskapi`) and, in shape, `internal/user/token_cache.go`'s `tokenCache` (short TTL, immediate same-process invalidation on write). The difference from `tokenCache` is what the TTL bounds: `tokenCache`'s 2s is a deliberately accepted staleness window; `cistern`'s `REDIS_CACHE_L1_TTL` (default `5s`) is the *residual* window only a lost `Bus` event falls back to — the common case invalidates immediately, cross-replica, over Redis Pub/Sub.

**Reads fail open too.** `buildCachedTaskRepository` (`cmd/api/main.go`) does not ping Redis at startup the way `openDatabase` pings `DATABASE_URL` — a genuine, deliberate asymmetry: PostgreSQL is this service's source of truth and there is nothing useful to do without it, but Redis is a cache `cistern` is built to fail open around (its own README: "Redis down ≠ app down"). An unreachable `REDIS_ADDR` surfaces as a slower `GET /v1/tasks` (every `FindAll` call misses L2 and reaches `next`), never a refusal to start or a `5xx`. `cistern.New` still returns an error for a genuine configuration mistake — an L1 TTL exceeding the overall TTL, already rejected earlier by `config.Load` independently — and that one does fail startup.

**The Redis instance is dedicated to this cache, configured `maxmemory-policy allkeys-lru`, and authenticates as a restricted ACL user (`cistern`).** `noeviction` — the policy `docs/DECISIONS.md` § "Backend do cairn.Store" chose for `cairn`'s durable link store — is the wrong policy here on purpose: for a cache, losing an entry under memory pressure must cost a miss, never a rejected write. `redisstore/README.md` documents both the policy and the ACL rule (`resetkeys ~cistern:* resetchannels &cistern:* -@all +get +set +del +mget +incr +pexpire +publish +subscribe`) as sufficient and tested; `docker-compose.yml`'s `redis` service and `k8s/25-redis.yaml`'s `Deployment` both apply it verbatim, verified against a real container and a real (throwaway) `kind` cluster respectively before being written here. The same documentation is explicit that this instance must never be shared with a future distributed rate limiter or with `cairn.Store`'s `redisstore` — `maxmemory-policy` applies to the whole instance, not per logical database — so either future use (see [Future Improvements](#future-improvements)) needs its own, separate instance.

**Nothing here is backed up.** `GET /v1/tasks`'s cache is never a source of truth (`cistern`'s own README states this as a hard property), so `docs/RUNBOOK-BACKUP-RESTORE.md` deliberately excludes it — losing the instance is a cold cache, never data loss; see that runbook's "Redis (cistern) — sem backup, e por quê" section for the recreation procedure instead.
...
- **Task search by title** — `GET /tasks` gained `status`/`priority` query-param filtering (Fase 14, `CI-14`/`CI-15` — see `docs/changes/frontend-redesign/`), and both parameters now accept several values at once (`?status=pending&status=done`); free-text search by title remains deferred, since nobody has asked for it yet. The natural point to also add a matching index once it is.
- **Distributed rate limiting** — all three tiers use `moat/ratelimit`'s in-process store, so each replica enforces its own budget and the effective limit behind a load balancer is N times the configured one. `ratelimit.WithStore` takes a shared backend (the library ships a Redis one as a separate module) for a deployment where that matters. Redis is provisioned in this project as of `docs/DECISIONS.md` § "Cache-aside para GET /v1/tasks" (for `cistern`'s `redisstore`+`Bus`), so the "no Redis infrastructure yet" reason this was deferred no longer holds — but switching this limiter to a shared store is still its own decision, not made by that one, and it would need its **own** Redis instance: `cistern`'s is configured `maxmemory-policy allkeys-lru` and its own documentation forbids sharing it with `moat`'s rate limiter (see that decision's correction paragraph) — this is not a cost-sharing opportunity, just a second instance to provision.
...
- **Password recovery, email verification, and TOTP second factor** (issues #221/#222/#225) — all three deferred together: the first two need real outbound email, which this project doesn't send today, and the third only makes sense once the first exists (2FA with no recovery path for a forgotten password is a second way to lose an account permanently, not a safety net). See `docs/DECISIONS.md`'s "Recuperação de senha, verificação de e-mail e segundo fator" for the reasoning and the trigger for revisiting.
...
- **`cairn.redisstore` as the link-shortening backend** — `LINK_SHORTENING_ENABLED` is currently backed by `memstore` (in-process, no external dependency), the deliberate first step of the rollout `docs/DECISIONS.md` § "Encurtador de links" describes. Swapping in `redisstore` needs Redis added to `docker-compose.yml` and `k8s/`, plus satisfying cairn's own operational contract for it (AOF `appendfsync everysec`, a replica, an exercised restore, `maxmemory-policy noeviction`). Redis itself stops being infrastructure this project doesn't carry once `docs/DECISIONS.md` § "Cache-aside para GET /v1/tasks" ships (provisioned there for `cistern`), but that instance cannot satisfy cairn's contract too — `cistern` requires `maxmemory-policy allkeys-lru` (the deliberate opposite of `noeviction`) and its own documentation rules out sharing it with `cairn.Store` outright, not as an option to weigh. This bullet still needs its own, separate Redis instance if it is ever picked up.

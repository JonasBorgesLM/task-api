...
**The key is `RemoteAddr`, never a forwarded header.** `X-Forwarded-For` and `X-Real-IP` are written by whoever sent the request, so keying on one lets a client mint a fresh identity per request and bypass the limit entirely. The consequence to be aware of when deploying: behind a reverse proxy, `RemoteAddr` is the *proxy's* address, so every client shares one global bucket. Either raise the global tier accordingly or teach the limiter which peers are trusted proxies — see Future Improvements.
...
- **Trusted-proxy awareness for the rate limiter** — the global tier keys on `RemoteAddr`, which behind a reverse proxy is the proxy's address, collapsing every client into one bucket. `moat/realip` derives a client address from forwarding headers *after* confirming the peer is a proxy you operate; wiring it in needs a config variable naming those peers, which this project does not have yet.

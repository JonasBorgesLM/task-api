...
│   ├── middleware/             # RequestID, Logging, Recovery, SecurityHeaders, CORS, RateLimiter, generic context helpers — no domain knowledge
│   ├── platform/
│   │   └── migrate/
...
- **Response security headers (`middleware.SecurityHeaders`)**: `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'`, `X-Frame-Options: DENY` and `X-Content-Type-Options: nosniff` on every response. The CSP is deliberately the tightest one expressible rather than an adaptation of web-application advice: this API only ever returns JSON, so there is no document here that legitimately loads a script or stylesheet, and a policy naming `script-src`/`style-src` would be strictly looser while protecting nothing extra. It still matters despite the JSON content type, because a browser coaxed into rendering a response as a document — a MIME-sniffing bug, a `Content-Type` an intermediary rewrote — then meets a policy forbidding it to execute whatever it found. `frame-ancestors 'none'` and `X-Frame-Options` say the same thing to two generations of browser; sending both costs one header. It sits outside `CORS` in the chain, and therefore outside everything that can answer without reaching the mux: a preflight `CORS` answers itself, `Recovery`'s `500` after a panic, and the router's own `404`/`405` are exactly the responses that would otherwise go out bare.
- **HSTS is opt-in (`HSTS_MAX_AGE`), and off by default.** Not an oversight, for two reasons. Per [RFC 6797 §7.2](https://www.rfc-editor.org/rfc/rfc6797#section-7.2) a user agent MUST ignore `Strict-Transport-Security` received over anything but a secure transport, and this binary always serves plain HTTP — the `scratch` image carries no certificates, and a real deployment terminates TLS at a proxy in front of it. The header would therefore be inert exactly where it is emitted and meaningful only on a hop this process cannot observe, so only the operator can know whether it is truthful. And HSTS is close to a one-way door: a client that cached `max-age=31536000` refuses plaintext to the host for a year, revocable only by serving HTTPS with `max-age=0` until every client has been seen again — not a default's decision to make on someone's behalf. The value is emitted as a bare `max-age`, with neither `includeSubDomains` nor `preload`: both reach past this service to every host under the domain, `preload` is in practice irreversible, and hardcoding them would leave an operator who wants HSTS for this host alone no way to say so.

...
- **`GET /v1/tasks` is fronted by a cache-aside layer (`cistern`, L1 in-process + L2 `redisstore` + `Bus`)** — see `docs/DECISIONS.md` § "Cache-aside para GET /v1/tasks" (including its correction paragraph). Every write that affects the listing (`CreateTask`, `UpdateTask`, `TransitionStatus`, `DeleteTask`) invalidates via `Bus` synchronously in the call, mirroring `cistern`'s own reference decorator (`examples/taskapi/taskapi.go`): a publish failure is **never** returned as a write failure — it is reported through an `OnInvalidationError` hook (wired in `cmd/api`, kept out of `Service`/`Handler`) and the worst case degrades to serving a stale L1 entry until its own TTL, exactly like `tokenCache` already tolerates for `ValidateToken`. Don't "fix" this by making an invalidation failure fail the write — that would let a Redis blip start rejecting task creates/updates, the opposite of `cistern`'s fail-open design. This Redis instance runs `maxmemory-policy allkeys-lru` (never `noeviction` — that is `cairn.Store`'s contract, for durable data, not a cache) and is never shared with `moat`'s rate limiter or `cairn.Store`. The `ETag` on `GET /v1/tasks` must keep hashing `(id, version)` of the rows actually returned regardless of whether they came from cache or `Repository` — the cache is never a second source for that hash.
